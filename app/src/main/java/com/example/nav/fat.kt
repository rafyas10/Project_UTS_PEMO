...
class fat : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        calculateButton.setOnClickListener {
            calculateRecommendedFat()
        }
    }

    private fun calculateRecommendedFat() {
        ...
        val fatRecommendation = calculateFatRecommendation(weight, activityLevel)

        resultTextView.text = "Recommended daily fat intake: ${"%.2f".format(fatRecommendation)} grams"
    }

    private fun calculateFatRecommendation(weight: Double, activityLevel: ActivityLevel): Double {
        // Fat recommendation formula based on activity level
        return when (activityLevel) {
            ActivityLevel.SEDENTARY -> weight * 0.3
            ActivityLevel.MODERATE -> weight * 0.35
            ActivityLevel.ACTIVE -> weight * 0.4
        }
    }
